...
class PairsBuilderDialogWindow(object):
    ...
    def __init__ (self, parentWindow, font=None, kernhash = None,
        ...
        self.w.gc.checkIncludeAlternates = CheckBox((leftX+2, blockY3, gcwx1, elemH),
        ...
        self.fontSuffixes = self.getListOfAllSuffixes()
        self.w.open()
    ...
    def makeGlyphsListWithSuffixes(self, glyphslist):
        glyphslistwithsuffixes = []
        for glyphname in glyphslist:
            glyphslistwithsuffixes.append(glyphname)
            for suffix in self.fontSuffixes:
                glyphwithsuffix = f"{glyphname}.{suffix}"
                if glyphwithsuffix in self.font:
                    glyphslistwithsuffixes.append(glyphwithsuffix)
        return glyphslistwithsuffixes
    ...
    def groupsDropCallback (self, sender, dropInfo):
        if dropInfo['isProposal']: pass
            ...
            if source != dest:
                if self.w.gc.checkIncludeAlternates.get():
                    glyphlist = self.makeGlyphsListWithSuffixes(glyphlist)
