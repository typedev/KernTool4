...
from tdKernToolEssentials4 import checkOverlapingGlyphs
...
# def checkOverlapingGlyphs (font, leftglyphname, rightglyphname, kernvalue):
# 	# RF3 style
# 	# if not leftglyph.rightMargin or not rightglyph.leftMargin: return False
# 	leftglyph = font[leftglyphname]
# 	rightglyph = font[rightglyphname]
...
# 		tempRightGlyph.moveBy((leftglyph.width + kernvalue, 0))
# 		tempglyph = tempLeftGlyph & tempRightGlyph
# 		if not tempglyph.isEmpty():
# 			return True
# 		return False
# 	except:
# 		# print ('error', leftglyph, rightglyph, kernvalue)
# 		return False

def checkOverlapingGlyphs_2 (font, l, r, kernvalue):
    return checkOverlapingGlyphs(font, font[l], font[r], kernvalue)
...
def autoCalcPairValue_2 (font, hashKernDic, pair): # mode = 'fixtouches'
    ...
    if checkOverlapingGlyphs_2(font, l, r, startKernValue):
        stepKern = 20
        countInc = 0
        while checkOverlapingGlyphs_2(font, l, r, startKernValue):
...
def processKernTouches(font, hashKernDic = None, pairsList = None): # samplefilepath = None, sfxlist = None
    if not pairsList:
        return
    # if not hashKernDic:
    # 	hashKernDic = TDHashGroupsDic()

    # hashKernDic.setFont(font)
    countPairsTotal = 0
    countPairsOverlapped = 0
    singlesPairs = 0
    grouppedPairs = 0
    report = []
    pairs2check = []
    ...
    for (l,r) in pairsList:
        ...
        countPairsTotal += 1
        pairinfo = researchPair(font, hashKernDic, (l, r))
        leftGroupName = pairinfo['L_realName']
        rightGroupName = pairinfo['R_realName']
        vt = pairinfo['kernValue']
        if not vt: vt = 0
        pairOverlaped = checkOverlapingGlyphs_2(font, l, r, vt-shiftKern)

        if pairOverlaped:
            if not monospacedFont:
                if hashKernDic.isKerningGroup(leftGroupName) and hashKernDic.isKerningGroup(rightGroupName):
                    report.append('++ %s %s == %s %s' % (leftGroupName, rightGroupName, l , r ))
                    grouppedPairs += 1
                    leftBaseGlyph = hashKernDic.getKeyGlyphByGroupname(leftGroupName)
                    rightBaseGlyph = hashKernDic.getKeyGlyphByGroupname(rightGroupName)
                    if leftBaseGlyph != l and rightBaseGlyph != r:
                        kv = autoCalcPairValue_2(font, hashKernDic, (l, r))
                        report.append('\t!! %s %s %i / %i' % (l, r, vt, kv))
                        font.kerning[(l, r)] = kv
                    elif leftBaseGlyph == l and rightBaseGlyph != r:
                        kv = autoCalcPairValue_2(font, hashKernDic, (l, r))
                        report.append('\t@! %s %s %i / %i' % (leftGroupName, r, vt, kv))
                        font.kerning[(leftGroupName, r)] = kv
                    elif leftBaseGlyph !=l and rightBaseGlyph == r:
                        kv = autoCalcPairValue_2(font, hashKernDic, (l, r))
                        report.append('\t!@ %s %s %i / %i' % (l, rightGroupName, vt, kv))
                        font.kerning[(l, rightGroupName)] = kv
                    elif leftBaseGlyph == l and rightBaseGlyph == r:
                        kv = autoCalcPairValue_2(font, hashKernDic, (l, r))
                        report.append('\t@@ %s %s %i / %i' % (l, r, vt, kv))
                        font.kerning[(leftGroupName, rightGroupName)] = kv


                elif hashKernDic.isKerningGroup(leftGroupName) and not hashKernDic.isKerningGroup(rightGroupName):
                    lbg = hashKernDic.getKeyGlyphByGroupname(leftGroupName)
                    pairinfo = researchPair(font, hashKernDic, (lbg, r))
                    _kv = pairinfo['kernValue']
                    if checkOverlapingGlyphs_2(font, lbg, r, _kv):
                        _kv = autoCalcPairValue_2(font, hashKernDic, (lbg, r))
                        report.append('@> %s %s %i / %i' % (leftGroupName, r, vt , _kv))
                        font.kerning[(leftGroupName, r)] = _kv
                    if checkOverlapingGlyphs_2(font, l, r, _kv):
                        _kv = autoCalcPairValue_2(font, hashKernDic, (l, r))
                        report.append('\t>> %s %s %i / %i' % (l, r, vt, _kv))
                        font.kerning[(l, r)] = _kv


                    # kv = autoCalcPairValue(font, hashKernDic, (l, r))
                    # report.append('>> %s %s %i / %i' % (leftGroupName, r, vt , kv))
                    # font.kerning[(leftGroupName, r)] = kv
                    # singlesPairs += 1

                elif not hashKernDic.isKerningGroup(leftGroupName) and hashKernDic.isKerningGroup(rightGroupName):
                    rbg = hashKernDic.getKeyGlyphByGroupname(rightGroupName)
                    pairinfo = researchPair(font, hashKernDic, (l, rbg))
                    _kv = pairinfo['kernValue']
                    if checkOverlapingGlyphs_2(font, l, rbg, _kv):
                        _kv = autoCalcPairValue_2(font, hashKernDic, (l, rbg))
                        report.append('<@ %s %s %i / %i' % (l, rightGroupName, vt, _kv))
                        font.kerning[(l, rightGroupName)] = _kv
                    if checkOverlapingGlyphs_2(font, l, r, _kv):
                        _kv = autoCalcPairValue_2(font, hashKernDic, (l, r))
                        report.append('\t<< %s %s %i / %i' % (l, r, vt, _kv))
                        font.kerning[(l, r)] = _kv
                ...
                elif not hashKernDic.isKerningGroup(leftGroupName) and not hashKernDic.isKerningGroup(rightGroupName):
                    kv = autoCalcPairValue_2(font, hashKernDic, (l,r))
                    report.append('-- %s %s %i / %i' % ( l , r , vt , kv))
                    font.kerning[(l,r)] = kv
                    singlesPairs += 1

            pairs2check.append((l, r))
            countPairsOverlapped += 1

    print('AutoCalc: Total checked:', countPairsTotal, 'Overlaps:', countPairsOverlapped, 'Groupped:', grouppedPairs, 'Singles:', singlesPairs)
...
class TDKernControl(object):
    def __init__ (self, fontsHashKernLib, glyphsView, groupsView = None, ON = False, scales = None):
        ...
        self.keyCommander.registerKeyCommand(KEY_E, shift = True, callback = self.makeAutoExceptionCallbak, callbackValue = 'side')
